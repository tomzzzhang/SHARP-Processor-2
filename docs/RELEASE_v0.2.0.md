...
**Last Updated:** 2026-07-06 PST — Claude (Mac session — **v0.2.3 shipped (macOS side)** (#56); this v0.2.0 record is unchanged. Prior — v0.2.2 macOS DMG built + published to the v0.2.2 release. Prior, 2026-06-26 — **macOS sign-off + release**. The #51 live-UI changes were verified on WKWebView (red checkmark checkbox toggles; gesture hint top-right beside the house = sole modebar button; greyscale User Manual mockup), and **both Mac DMGs were built, integrity/signature-verified, and published to GitHub releases**: `SHARP.Processor.2_0.1.13_aarch64.dmg` → v0.1.13 (the release's **first-ever Mac build** — Windows-only before) and `SHARP.Processor.2_0.2.0_aarch64.dmg` → the v0.2.0 pre-release. Both releases now carry aarch64 DMG + x64/x86 Windows installers. **Merged to `main`** via PR #10 (merge commit `4c10627`). Prior, on branch `feature/ui-ux-enhancements` — post-v0.2.0 UI/UX pass on top of the shipped multichannel core. **#51 live-UI tweaks** — binary **toggles → native checkbox** (red checkmark box, reverting #49's radio-dot; tri-state per-well auto-baseline stays an `indeterminate` checkbox); the **plot gesture-hint relocated to the top-right beside the reset/"house" button** (natural ~house-icon size, theme-aware translucent chip, `displaylogo:false`, #50's `extraBottom` reservation reverted — resolving the original hint↔x-label collision); the in-app **User Manual** mockup updated to match + greyscaled per request. Earlier batches: restyle / icon-discipline / a11y (#48); live-UI feedback + previously-deferred behaviour items (#49); responsive plot axis-label scaling (#50). All verified `tsc -b` + `vite build` clean, eslint 50=50 (zero new). Multichannel v0.2.0 core + on-disk format unchanged (presentation only). Committed + pushed (`f3e0f4b`); Windows x64 + x86 0.2.0 installers rebuilt (`dist-release/`); Mac DMG builds prepared via OneDrive `MAC_BUILD_HANDOFF.md` (v0.1.13 + 0.2.0). Live-UI sign-off **PASSED on macOS**; both DMGs published (see top of this entry). Prior: batch 4 perf sweep #46 / §16. See CLAUDE.md table #48–51.)
