...
**Last Updated:** 2026-06-01 PST — Claude (v0.2.0 **SHIPPED** as a GitHub pre-release (v0.1.13 stays "Latest"); post-release beta fixes (CLAUDE.md #47 — legend "Selected wells only" default-off, separate-by-color clears per-curve overrides, README matter-of-fact) committed to `main` and the beta installers refreshed in place. Earlier, RELEASE PREP: user-facing docs updated (README / in-app UserManual / RELEASE_NOTES) + cautious **pre-release** rollout decided (v0.1.13 stays "Latest"; Windows-first; Mac DMGs from the tags later) — see STATUS.md top entry / CLAUDE.md #46. v0.2.0 multichannel write-up + handoff; + post-review bug-fix pass §12–13; + styling & plot-UX pass §14; + batch 2 §15 single-channel VIEW MODE; + batch 3 §16 curve-centric selection / styling / grouping + results-table tree; + **batch 4 — performance sweep** (no behaviour change: memoized `usePlotStyle` + data-keyed `datarevision` so Plotly does cheap restyles not full replots on hover/selection; per-well analysis computed once via a shared `AnalysisResultsProvider` context instead of in ~8 components; threshold/box-select drags coalesced to `requestAnimationFrame` — full detail in CLAUDE.md table #46). Branch uncommitted; tsc + vite + 12/12 regression green; live-UI sign-off pending. See CLAUDE.md table #46 / §16 "State for the next session")
