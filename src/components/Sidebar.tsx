...
const TAB_TRIGGER_CLASS =
  'text-xs font-semibold tracking-wide uppercase after:!bg-[var(--brand-red-mid)] data-active:!text-[var(--brand-red-dark)]';
  ...
  return (
    <div className="w-full h-full bg-background flex flex-col overflow-hidden">
      <Tabs defaultValue="data" className="flex flex-col h-full">
        ...
        >
          <TabsTrigger value="data" disabled={!hasExperiment} className={`${TAB_TRIGGER_CLASS} ${!hasExperiment ? 'opacity-50 cursor-default' : ''}`}>Data</TabsTrigger>
          <TabsTrigger value="wells" disabled={!hasExperiment} className={`${TAB_TRIGGER_CLASS} ${!hasExperiment ? 'opacity-50 cursor-default' : ''}`}>Wells</TabsTrigger>
          <TabsTrigger value="analysis" disabled={!hasExperiment} className={`${TAB_TRIGGER_CLASS} ${!hasExperiment ? 'opacity-50 cursor-default' : ''}`}>Analysis</TabsTrigger>
          <TabsTrigger value="style" disabled={!hasExperiment} className={`${TAB_TRIGGER_CLASS} ${!hasExperiment ? 'opacity-50 cursor-default' : ''}`}>Style</TabsTrigger>
        </TabsList>
        {!hasExperiment ? (
